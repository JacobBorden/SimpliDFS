...
// Let's create a metadata service that will act as the core to track file
...
static bool verify_quote(const std::string &quote) {
...
// Constructor, registerNode, processHeartbeat, checkForDeadNodes,
// printMetadata, saveMetadata, loadMetadata are assumed to be already
// implemented (or their stubs are sufficient for now). We will focus on
// implementing the new/modified methods for FUSE operations.

// Modified addFile to include mode and return an error code
int MetadataManager::addFile(const std::string &filename,
  ...
  if (fileMetadata.count(filename)) {
    Logger::getInstance().log(
        LogLevel::WARN,
        "[MetadataManager] addFile: File already exists: " + filename);
    return EEXIST;
  }
  ...
  for (const auto &nodeID : preferredNodes) {
    ...
    if (it != registeredNodes.end() && it->second.isAlive &&
      ...
      if (std::find(targetNodes.begin(), targetNodes.end(), nodeID) ==
          targetNodes.end()) {
  ...
  for (size_t i = 0; i < targetNodes.size(); ++i) {
    ...
    try {
      ...
      Logger::getInstance().log(
          LogLevel::ERROR,
          "[MetadataManager] Failed to send create command to node " + nodeID +
              ": " + e.what());
  ...
  if (successes.size() < DEFAULT_REPLICATION_FACTOR) {
    ...
    for (size_t i = 0; i < successes.size(); ++i) {
      ...
      try {
        ...
        Logger::getInstance().log(
            LogLevel::ERROR,
            "[MetadataManager] Failed rollback delete to node " + nodeID +
                ": " + e.what());
        healthCache_.recordFailure(nodeID);
      }
    }
  ...
  oss_add << "[MetadataManager] File " << filename << " added with mode "
          << std::oct << mode << std::dec << ". Assigned to nodes: ";
...
bool MetadataManager::removeFile(const std::string &filename) {
  ...
  for (size_t i = 0; i < nodesToNotify.size(); ++i) {
    ...
    try {
      ...
      Logger::getInstance().log(
          LogLevel::ERROR,
          "[MetadataManager] Failed to send delete command to node " + nodeID +
              ": " + e.what());
      healthCache_.recordFailure(nodeID);
    }
  }
...
int MetadataManager::getFileAttributes(const std::string &filename,
  ...
  oss_attr << "[MetadataManager] getFileAttributes for " << filename
           << ": mode=" << std::oct << mode << std::dec << ", size=" << size;
...
int MetadataManager::checkAccess(const std::string &filename,
  ...
  Logger::getInstance().log(LogLevel::DEBUG,
                            "[MetadataManager] checkAccess for " + filename +
                                " (mask: " + std::to_string(access_mask) +
                                ") passed.");
  return 0;
}
...
int MetadataManager::openFile(const std::string &filename, uint32_t flags) {
  ...
  Logger::getInstance().log(LogLevel::DEBUG,
                            "[MetadataManager] openFile for " + filename +
                                " (flags: " + std::to_string(flags) +
                                ") succeeded.");
  return 0;
}

int MetadataManager::readFileData(const std::string &filename, int64_t offset,
  ...
  if (nodeAddrs.empty()) {
    Logger::getInstance().log(
        LogLevel::ERROR,
        "[MetadataManager] readFileData: no available replicas for " +
            filename);
    return ERR_NO_REPLICA;
  }
  ...
  for (const auto &entry : nodeAddrs) {
    ...
    try {
      ...
      Logger::getInstance().log(LogLevel::ERROR,
                                "[MetadataManager] Failed to read from node " +
                                    nodeID + ": " + e.what());
      healthCache_.recordFailure(nodeID);
    }
  }
  ...
  Logger::getInstance().log(LogLevel::DEBUG,
                            "[MetadataManager] readFileData for " + filename +
                                " from " + usedNode + ": returned " +
                                std::to_string(out_size_read) + " bytes.");
  return 0;
}

int MetadataManager::writeFileData(const std::string &filename, int64_t offset,
  ...
  if (offset < 0)
    offset = 0; // Treat negative offset as 0 for this logic
  ...
  if (!fileSizes.count(filename) || write_end_offset > fileSizes.at(filename)) {
    ...
    Logger::getInstance().log(LogLevel::INFO,
                              "[MetadataManager] File " + filename +
                                  " size updated to " +
                                  std::to_string(fileSizes[filename]));
    markDirty();
  }
  ...
  if (!nodeAddrs.empty()) {
    ...
    try {
      ...
      Logger::getInstance().log(
          LogLevel::ERROR,
          "[MetadataManager] Failed to send write to primary node " +
              primaryID + ": " + e.what());
    ...
    for (size_t i = 1; i < nodeAddrs.size(); ++i) {
      ...
      int replicaPort =
          std::stoi(replicaAddr.substr(replicaAddr.find(':') + 1));
      ...
      try {
        ...
        Logger::getInstance().log(
            LogLevel::ERROR,
            "[MetadataManager] Failed to send replicate command to " +
                primaryID + ": " + e.what());
        healthCache_.recordFailure(primaryID);
      }
      ...
      try {
        ...
        Logger::getInstance().log(
            LogLevel::ERROR,
            "[MetadataManager] Failed to send receive command to " + replicaID +
                ": " + e.what());
...
int MetadataManager::renameFileEntry(const std::string &old_filename,
  ...
  if (node_fh.empty())
    return ENOENT; // Should not happen if count check passed
  ...
  Logger::getInstance().log(LogLevel::INFO, "[MetadataManager] Renamed " +
                                                old_filename + " to " +
                                                new_filename);
  if (raftNode_) {
...
bool MetadataManager::applySnapshotDelta(const std::string &nodeIdentifier,
  ...
  while (std::getline(ss, line)) {
    if (line.rfind("Added: ", 0) == 0) {
      ...
      if (!file.empty() && fileExists(file)) {
        Logger::getInstance().log(LogLevel::INFO,
                                  "[MetadataManager] Marking modified file " +
                                      file + " from node " + nodeIdentifier);
        changed = true;
      }
    }
  }
...
// fileMetadata and registeredNodes. Their definitions reside in the header
// file.
...
void HandleClientConnection(Networking::Server &server_instance,
  ...
  std::cerr << "DIAGNOSTIC: HandleClientConnection: Thread started for client "
            << server_instance.GetClientIPAddress(_pClient) << std::endl;
  std::string client_ip_str = server_instance.GetClientIPAddress(
      _pClient); // Store for logging after disconnect
    ...
    try {
      Logger::getInstance().log(
          ...
          "Handling client connection from " + client_ip_str +
              ", active clients: " +
              std::to_string(metadataManager.getActiveClientCount()));
      std::vector<char> received_vector = server_instance.Receive(_pClient);
      std::cerr << "DIAGNOSTIC: HandleClientConnection: Received "
                << received_vector.size() << " bytes from client." << std::endl;
      if (received_vector.empty()) {
        Logger::getInstance().log(
            LogLevel::INFO,
            "Client " + client_ip_str +
                " disconnected or receive failed. Closing connection.");
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Thread finishing due "
                     "to empty receive for client "
                  << client_ip_str << std::endl;
        break;
      }
      std::string received_data_str(received_vector.begin(),
                                    received_vector.end());
      Logger::getInstance().log(LogLevel::DEBUG, "Received data from " +
                                                     client_ip_str + ": " +
                                                     received_data_str);
      ...
      try {
        ...
        std::cerr
            << "DIAGNOSTIC: HandleClientConnection: Deserialized request type "
            << static_cast<int>(request._Type) << " for path '" << request._Path
            << "' from client." << std::endl;
      } catch (const std::runtime_error &re) {
        Logger::getInstance().log(LogLevel::ERROR,
                                  "Failed to deserialize message from " +
                                      client_ip_str + ": " + re.what() +
                                      ". Closing connection.");
        break; // Critical error, terminate connection handling
      }

      bool shouldSave = false;
      Message response_to_send; // To hold the message to be sent, avoids
                                // serializing multiple times if not needed for
                                // some paths.

      switch (request._Type) {
      case MessageType::GetAttr: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                     "GetAttr for path '"
                  << request._Path << "'" << std::endl;
        Logger::getInstance().log(LogLevel::INFO,
                                  "[Metaserver] Received GetAttr for: " +
                                      request._Path);
        Message res_msg;
        ...
        if (request._Path == "/") {
          ...
          res_msg._ErrorCode = metadataManager.getFileAttributes(
              norm_path, res_msg._Mode, res_msg._Uid, res_msg._Gid,
              res_msg._Size);
        }
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case GetAttr, path '"
                  << request._Path << "'" << std::endl;
      ...
      case MessageType::Readdir: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                  ...
                  << request._Path << "'" << std::endl;
        Logger::getInstance().log(LogLevel::INFO,
                                  "[Metaserver] Received Readdir for: " +
                                      request._Path);
        Message res_msg;
        ...
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case Readdir, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        break;
      }
      case MessageType::Access: {
        ...
        Logger::getInstance().log(
            LogLevel::INFO,
            "[Metaserver] Received Access for: " + request._Path +
                " with mode " + std::to_string(request._Mode));
        Message res_msg;
        ...
        if (request._Path == "/") {
          ...
          std::string norm_path = normalize_path_to_filename(request._Path);
          res_msg._ErrorCode = metadataManager.checkAccess(
              norm_path, static_cast<uint32_t>(request._Mode));
        }
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case Access, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        break;
      }
      case MessageType::Open: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case Open "
                     "for path '"
                  << request._Path << "'" << std::endl;
        Logger::getInstance().log(
            LogLevel::INFO, "[Metaserver] Received Open for: " + request._Path +
                                " with flags " + std::to_string(request._Mode));
        Message res_msg;
        ...
        if (request._Path == "/") {
          ...
          // This open is more about checking if file exists and can be opened
          // based on flags (RDONLY, WRONLY, etc.) which checkAccess implicitly
          // does for now.
          res_msg._ErrorCode = metadataManager.openFile(
              norm_path, static_cast<uint32_t>(request._Mode));
        }
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case Open, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        break;
      }
      case MessageType::CreateFile: {
        ...
        oss_create << "[Metaserver] Received CreateFile for: " << request._Path
                   << " with mode " << std::oct << request._Mode << std::dec;
        Logger::getInstance().log(LogLevel::INFO, oss_create.str());
        Message res_msg;
        res_msg._Type = MessageType::CreateFileResponse;
        std::string norm_path_filename =
            normalize_path_to_filename(request._Path);

        if (norm_path_filename == "/" || norm_path_filename.empty() ||
            norm_path_filename.rfind('/') != std::string::npos) {
          res_msg._ErrorCode = EINVAL;
        } else {
          // Preferred nodes list is empty for now, can be enhanced later
          res_msg._ErrorCode = metadataManager.addFile(
              norm_path_filename, {}, static_cast<uint32_t>(request._Mode));
          if (res_msg._ErrorCode == 0) {
            shouldSave = true;
          }
        }
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case CreateFile, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        break;
      }
      case MessageType::ReadFile: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                  ...
                  << request._Path << "'" << std::endl;
        Logger::getInstance().log(
            LogLevel::INFO, "[Metaserver] Received Read for: " + request._Path +
                                " Offset: " + std::to_string(request._Offset) +
                                " Size: " + std::to_string(request._Size));
        Message res_msg;
        res_msg._Type = MessageType::ReadResponse;
        std::string norm_path_filename =
            normalize_path_to_filename(request._Path);

        res_msg._ErrorCode = metadataManager.readFileData(
            norm_path_filename, request._Offset, request._Size, res_msg._Data,
            res_msg._Size);
        // readFileData sets res_msg._Size to actual bytes read/to be sent
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case ReadFile, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        break;
      }
      case MessageType::WriteFile: {
        ...
        Logger::getInstance().log(
            LogLevel::INFO,
            "[Metaserver] Received Write for: " + request._Path +
                " Offset: " + std::to_string(request._Offset) +
                " Size: " + std::to_string(request._Size) +
                " DataLen: " + std::to_string(request._Data.length()));
        Message res_msg;
        ...
        res_msg._ErrorCode = metadataManager.writeFileData(
            norm_path_filename, request._Offset, request._Data,
            bytes_written_confirmed);
        if (res_msg._ErrorCode == 0) {
          res_msg._Size = bytes_written_confirmed; // Set the size in response
                                                   // to what was "written"
          shouldSave = true;
        } else {
        ...
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case WriteFile, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        break;
      }
      case MessageType::TruncateFile: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                     "TruncateFile for path '"
                  << request._Path << "'" << std::endl;
        Logger::getInstance().log(
            LogLevel::INFO,
            "[Metaserver] Received TruncateFile for: " + request._Path +
                " size " + std::to_string(request._Size));
        Message res_msg;
        res_msg._Type = MessageType::TruncateFileResponse;
        std::string norm_path = normalize_path_to_filename(request._Path);
        res_msg._ErrorCode =
            metadataManager.truncateFile(norm_path, request._Size);
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case TruncateFile, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        if (res_msg._ErrorCode == 0)
          shouldSave = true;
      ...
      case MessageType::Unlink: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                  ...
                  << request._Path << "'" << std::endl;
        Logger::getInstance().log(LogLevel::INFO,
                                  "[Metaserver] Received Unlink for: " +
                                      request._Path);
        Message res_msg;
      ...
      case MessageType::Rename: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                  ...
                  << request._Path << "'" << std::endl;
        Logger::getInstance().log(
            LogLevel::INFO, "[Metaserver] Received Rename for: " +
                                request._Path + " to " + request._NewPath);
        Message res_msg;
        res_msg._Type = MessageType::RenameResponse;
        std::string norm_old_filename =
            normalize_path_to_filename(request._Path);
        std::string norm_new_filename =
            normalize_path_to_filename(request._NewPath);

        bool invalid_paths =
            norm_old_filename == "/" || norm_old_filename.empty() ||
            norm_old_filename.rfind('/') != std::string::npos ||
            norm_new_filename == "/" || norm_new_filename.empty() ||
            norm_new_filename.rfind('/') != std::string::npos;

        if (invalid_paths) {
          res_msg._ErrorCode = EINVAL;
        } else {
          res_msg._ErrorCode = metadataManager.renameFileEntry(
              norm_old_filename, norm_new_filename);
          if (res_msg._ErrorCode == 0) {
            shouldSave = true;
          }
        }
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case Rename, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        break;
      }
      ...
      case MessageType::RegisterNode: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                     "RegisterNode for node '"
                  << request._Filename << "'" << std::endl;
        Message reg_res;
        reg_res._Type = MessageType::RegisterNode;
        if (!verify_quote(request._Data)) {
          reg_res._ErrorCode = EPERM;
        } else {
          metadataManager.registerNode(request._Filename, request._NodeAddress,
                                       request._NodePort);
          shouldSave = true;
          reg_res._ErrorCode = 0;
        }
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case RegisterNode, node '"
                  << request._Filename << "'" << std::endl;
        server_instance.Send(Message::Serialize(reg_res).c_str(), _pClient);
        Logger::getInstance().log(LogLevel::INFO,
                                  "Sent registration confirmation to node " +
                                      request._Filename);
        break;
      }
      case MessageType::GetFileNodeLocationsRequest: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                     "GetFileNodeLocationsRequest for path '"
                  << request._Path << "'" << std::endl;
        Logger::getInstance().log(
            LogLevel::INFO,
            "[Metaserver] Received GetFileNodeLocationsRequest for: " +
                request._Path);
        Message res_msg;
        ...
        if (norm_path.empty()) {
          res_msg._ErrorCode =
              EINVAL; // Invalid argument if path is empty or just "/"
        } else {
          try {
            std::vector<std::string> node_ids =
                metadataManager.getFileNodes(norm_path);
            if (node_ids.empty()) {
              // This case might mean file exists but has no nodes (should
              // ideally not happen if addFile ensures nodes) Or getFileNodes
              // throws if file doesn't exist, so this path might not be hit for
              // "no nodes".
              Logger::getInstance().log(
                  LogLevel::WARN, "[Metaserver] File " + norm_path +
                                      " found but has no associated nodes.");
              res_msg._ErrorCode =
                  ENOENT; // Or a more specific error like ENODATA
            } else {
              ...
              for (size_t i = 0; i < node_ids.size(); ++i) {
                try {
                  ...
                  // Log that a specific node ID couldn't be resolved, but
                  // continue if possible
                  Logger::getInstance().log(
                      LogLevel::ERROR,
                      "[Metaserver] Error getting info for node ID " +
                          node_ids[i] + " for file " + norm_path + ": " +
                          re_node.what());
                  // Depending on desired behavior, could set an error here or
                  // just skip this node
                }
              }
              ...
              if (res_msg._Data.empty() && !node_ids.empty()) {
                ...
                Logger::getInstance().log(LogLevel::ERROR,
                                          "[Metaserver] Failed to resolve any "
                                          "node addresses for file " +
                                              norm_path);
                res_msg._ErrorCode =
                    EHOSTUNREACH; // Or some other suitable error
              } else if (res_msg._Data.empty() && node_ids.empty()) {
                // Explicitly set ENOENT if no nodes were found (e.g. file
                // exists but node list is empty)
                res_msg._ErrorCode = ENOENT;
              } else {
            ...
            Logger::getInstance().log(
                LogLevel::WARN, "[Metaserver] GetFileNodeLocationsRequest: "
                                "File not found or error for " +
                                    norm_path + ": " + re_file.what());
            res_msg._ErrorCode = ENOENT; // File not found
          }
        }
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case GetFileNodeLocationsRequest, path '"
                  << request._Path << "'" << std::endl;
        server_instance.Send(Message::Serialize(res_msg).c_str(), _pClient);
        break;
      }
      ...
      case MessageType::SnapshotDelta: {
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                     "SnapshotDelta from node '"
                  << request._Filename << "'" << std::endl;
        bool changed = metadataManager.applySnapshotDelta(request._Filename,
                                                          request._Content);
        Message res;
        res._Type = MessageType::SnapshotDelta;
        res._ErrorCode = 0;
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case SnapshotDelta"
                  << std::endl;
      ...
      case MessageType::DeleteFile: { // Legacy DeleteFile, treat like unlink
        ...
        Logger::getInstance().log(
            LogLevel::INFO,
            "[Metaserver] Received (legacy) DeleteFile request for " +
                request._Filename);
        Message del_res;
        del_res._Type = MessageType::FileRemoved;
        std::string file_to_delete =
            normalize_path_to_filename(request._Filename);

        if (file_to_delete == "/" || file_to_delete.empty() ||
            file_to_delete.rfind('/') != std::string::npos) {
        ...
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case DeleteFile, path '"
                  << request._Filename << "'" << std::endl;
        server_instance.Send(Message::Serialize(del_res).c_str(), _pClient);
        Logger::getInstance().log(
            LogLevel::INFO,
            "[Metaserver] Sent DeleteFile processing result for " +
                file_to_delete);
        break;
      }
      ...
      default:
        std::cerr << "DIAGNOSTIC: HandleClientConnection: Processing case "
                     "Default (Unknown Type) for type "
                  << static_cast<int>(request._Type) << std::endl;
        Logger::getInstance().log(
            LogLevel::WARN,
            "Received unhandled or unknown message type: " +
                std::to_string(static_cast<int>(request._Type)) +
                " from client " + server_instance.GetClientIPAddress(_pClient));
        Message err_res;
        err_res._Type = request._Type;
        err_res._ErrorCode = ENOSYS;
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
                     "response for case Default (Unknown Type)"
                  << std::endl;
      ...
      if (shouldSave) {
        Logger::getInstance().log(
            LogLevel::DEBUG,
            "[Metaserver] Metadata modified, marking as dirty.");
        metadataManager.markDirty();
      }
    } catch (const Networking::NetworkException &ne) {
      std::cerr
          << "DIAGNOSTIC: HandleClientConnection: NetworkException caught: "
          << ne.what() << " for client " << client_ip_str << std::endl;
      Logger::getInstance().log(
          LogLevel::ERROR, "Network error in HandleClientConnection for " +
                               client_ip_str + ": " + std::string(ne.what()));
      break; // Exit loop on network error
    } catch (const std::runtime_error &re) {
      std::cerr << "DIAGNOSTIC: HandleClientConnection: runtime_error caught: "
                << re.what() << " for client " << client_ip_str << std::endl;
      Logger::getInstance().log(
          LogLevel::ERROR, "Runtime error (e.g., deserialization) in "
                           "HandleClientConnection for " +
                               client_ip_str + ": " + std::string(re.what()));
      break; // Exit loop on runtime error
    } catch (const std::exception &e) {
      std::cerr << "DIAGNOSTIC: HandleClientConnection: std::exception caught: "
                << e.what() << " for client " << client_ip_str << std::endl;
      Logger::getInstance().log(
          LogLevel::ERROR, "Generic exception in HandleClientConnection for " +
                               client_ip_str + ": " + std::string(e.what()));
      break; // Exit loop on generic error
    } catch (...) {
      std::cerr << "DIAGNOSTIC: HandleClientConnection: UNHANDLED EXCEPTION "
                   "CAUGHT IN THREAD for client "
                << client_ip_str << ". Thread terminating." << std::endl;
      Logger::getInstance().log(
          LogLevel::ERROR,
          "Unknown exception in HandleClientConnection for " + client_ip_str);
  ...
  try {
    ...
    std::cerr << "DIAGNOSTIC: HandleClientConnection: Client " << client_ip_str
              << " disconnected." << std::endl;
    Logger::getInstance().log(LogLevel::INFO,
                              "Disconnected client " + client_ip_str);
  } catch (const Networking::NetworkException &ne) {
    std::cerr << "DIAGNOSTIC: HandleClientConnection: NetworkException during "
                 "DisconnectClient for "
              << client_ip_str << ": " << ne.what() << std::endl;
    Logger::getInstance().log(
        LogLevel::ERROR, "Network error during DisconnectClient for " +
                             client_ip_str + ": " + std::string(ne.what()));
  }

  metadataManager.unregisterClientThread(std::this_thread::get_id());
  std::cerr
      << "DIAGNOSTIC: HandleClientConnection: Thread finishing for client "
      << client_ip_str << std::endl;
}
...
void MetadataManager::applyRaftCommand(const std::string &cmd) {
  if (cmd.rfind("ADD|", 0) == 0) {
    ...
    if (pos == std::string::npos)
      return;
    std::string file = cmd.substr(4, pos - 4);
    uint32_t mode = static_cast<uint32_t>(std::stoul(cmd.substr(pos + 1)));
    std::lock_guard<std::mutex> lock(metadataMutex);
    if (!fileMetadata.count(file)) {
      fileMetadata[file] = {};
      fileModes[file] = mode;
      fileSizes[file] = 0;
      fileHashes[file] = "";
    ...
    std::lock_guard<std::mutex> lock(metadataMutex);
    if (fileMetadata.count(oldf) && !fileMetadata.count(newf)) {
      auto fh = fileMetadata.extract(oldf);
      fh.key() = newf;
      fileMetadata.insert(std::move(fh));
      if (fileModes.count(oldf)) {
        auto mh = fileModes.extract(oldf);
        mh.key() = newf;
        fileModes.insert(std::move(mh));
      }
      if (fileSizes.count(oldf)) {
        auto sh = fileSizes.extract(oldf);
        sh.key() = newf;
        fileSizes.insert(std::move(sh));
      }
      if (fileHashes.count(oldf)) {
        auto hh = fileHashes.extract(oldf);
        hh.key() = newf;
        fileHashes.insert(std::move(hh));
      }
    }
  } else if (cmd.rfind("ROOT|", 0) == 0) {
    currentRootCid_ = cmd.substr(5);
...
void MetadataManager::markDirty() {
  ...
  ChunkStore store;
  currentRootCid_ = MerkleTree::hashDirectory(entries, store);
  MetricsRegistry::instance().setGauge("simplidfs_merkle_root", 1,
                                       {{"cid", currentRootCid_}});
  if (raftNode_) {
    raftNode_->appendCommand("ROOT|" + currentRootCid_);
  }
}
