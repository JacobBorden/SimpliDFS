  ...
  while (true) { // Loop to handle multiple requests
    try {
      ...
      case MessageType::RegisterNode: {
        ...
        const char *expected = std::getenv("EXPECTED_PCR7_QUOTE");
        bool quote_ok = true;
        if (expected && *expected) {
          quote_ok = request._Data == expected;
        }
        Message reg_res;
        ...
        if (quote_ok) {
          ...
          reg_res._ErrorCode = EACCES;
          Logger::getInstance().log(LogLevel::WARN,
                                    "Rejecting node " + request._Filename +
                                        " due to invalid PCR7 quote");
        }
        std::cerr << "DIAGNOSTIC: HandleClientConnection: About to send "
