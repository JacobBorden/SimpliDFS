...
std::string HandleGetRequest(const HTTPREQUEST &request) {
  ...
  responseStream << httpResponse.protocol << " "
                 << httpResponse.statusCodeNumber << " "
...
std::string HandlePostRequest(const HTTPREQUEST &request) {
  ...
  httpResponse.body = "<html><body><h1>POST Data Received</h1><pre>" +
...
std::string HandlePutRequest(const HTTPREQUEST &request) {
  ...
  httpResponse.contentType = "text/html";
  httpResponse.body =
      "<html><body><h1>Data Processed Successfully</h1></body></html>";
...
std::string GenerateHttpRequestString(const HTTPREQUEST &request) {
  ...
  requestStream << HttpMethodToString(request.method) << " " << request.uri
                << " " << request.protocol << "\r\n";
...
HTTPRESPONSE SendHttpRequestSSL(const HTTPREQUEST &, const std::string &,
                                const std::string &) {
  throw std::runtime_error("SendHttpRequestSSL not implemented");
}
