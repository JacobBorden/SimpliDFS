...
void writer_thread_func(int thread_id) {
  ...
  std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
            << " TID: " << std::this_thread::get_id() << "] Thread "
            << thread_id << ": Attempting to open file " << FULL_TEST_FILE_PATH
            << std::endl;
  ...
  std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
            << " TID: " << std::this_thread::get_id() << "] Thread "
            << thread_id << ": File opened successfully." << std::endl;
  ...
  for (int i = 0; i < NUM_LINES_PER_THREAD; ++i) {
    ...
    std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
              << " TID: " << std::this_thread::get_id() << "] Thread "
              << thread_id << ": Line " << i << ", seeking to offset " << offset
              << std::endl;
    ...
    if (outfile.fail()) {
      std::cerr << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
                ...
                << " failed. Stream state: " << outfile.rdstate() << std::endl;
    }

    std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
              << " TID: " << std::this_thread::get_id() << "] Thread "
              << thread_id << ": Line " << i << ", writing "
              << line_to_write.length() << " bytes." << std::endl;
    outfile.write(line_to_write.c_str(), line_to_write.length());
    if (outfile.fail()) {
      std::cerr << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
                << " TID: " << std::this_thread::get_id() << "] Thread "
                << thread_id << ": Write failed at offset " << offset
                << ". Stream state: " << outfile.rdstate() << std::endl;
    }
  }
  std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
            << " TID: " << std::this_thread::get_id() << "] Thread "
            << thread_id << ": All lines written. Closing file." << std::endl;
  outfile.close();
  std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
            << " TID: " << std::this_thread::get_id() << "] Thread "
...
int main() {
  ...
  if (bytes_read != remaining_bytes) {
    std::cerr << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
              ...
              << " bytes but got " << bytes_read << std::endl;
  }
