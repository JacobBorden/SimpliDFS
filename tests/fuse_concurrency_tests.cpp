...
// The primary goal is to ensure the filesystem behaves correctly and consistently
// when multiple threads perform simultaneous file operations.
//
...
// This helps in correlating events from different threads with millisecond precision.
...
// Barrier for synchronizing worker threads (both random write and append tests).
// These primitives ensure that all threads are created and ready to start
// their respective file operations (writing or appending) simultaneously.
// This maximizes the chances of race conditions and concurrency issues surfacing.
std::mutex start_mutex; // Mutex to protect access to start_count and condition variable.
std::condition_variable start_cv; // Condition variable to signal threads to start.
int start_count = 0; // Counter for threads that have reached the barrier.

// Helper function to generate a unique and deterministic string for each line in the random write test.
// The content is based on the thread ID and line number, ensuring that each line
// has a predictable payload. This is crucial for verifying data integrity after
// concurrent writes, as the test can check if the correct data was written to the correct offset.
...
// Its primary goal is to write a specific number of lines (NUM_LINES_PER_THREAD)
// to pre-determined, distinct offsets within the test file (FULL_TEST_FILE_PATH).
// This tests the FUSE filesystem's ability to handle concurrent writes to different
// parts of a file without data corruption or interference between threads.
//
// The extensive std::cout and std::cerr statements throughout this function are for
// detailed logging of thread actions, timings, and potential errors. This logging
// is invaluable for debugging concurrency issues by providing a trace of each
// thread's operations and their outcomes.
void writer_thread_func(int thread_id) {
  ...
  // before any of them start their file operations. This is achieved using a mutex
  // (start_mutex), a condition variable (start_cv), and a counter (start_count).
  // The last thread to increment start_count notifies all other waiting threads.
  {
    std::unique_lock<std::mutex> lk(start_mutex);
    start_count++;
    if (start_count == NUM_THREADS) {
      // This thread is the last one to reach the barrier, so it notifies others.
      std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
                << " TID: " << std::this_thread::get_id() << "] Thread "
...
// Its goal is to concurrently append a specific number of lines (NUM_LINES_PER_APPEND_THREAD)
// to a shared test file (FULL_APPEND_TEST_FILE_PATH). This tests the atomicity and
// correctness of append operations under concurrent load.
//
// The std::cout and std::cerr statements provide detailed logging for debugging,
// tracking each thread's progress and any errors encountered during append operations.
void appender_thread_func(int thread_id) {
  ...
  // Barrier synchronization:
  // Similar to writer_thread_func, this ensures all appender threads start simultaneously.
  // The start_count is typically reset in run_append_test before launching these threads.
  // This synchronized start is crucial for maximizing contention on the file.
  {
    std::unique_lock<std::mutex> lk(start_mutex);
    start_count++;
...
// Verifies that the FUSE filesystem's mount point (MOUNT_POINT) is available and operational.
// This function is a prerequisite for running the actual test logic, ensuring that
// the target filesystem is ready for interaction. It logs its actions and any errors
// encountered, aiding in diagnosing setup issues if the tests cannot start.
// Returns true if the mount point is ready, false otherwise.
bool check_mount_point_ready() {
  struct stat sb; // Used to store information about the mount point file/directory.
  ...
  // externally and needs a moment to fully initialize. It's a pragmatic approach
  // to potentially improve reliability in some environments.
  std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
            << " TID: " << std::this_thread::get_id()
  ...
  // We check if stat() succeeds (returns 0) and if the entry is a directory (S_ISDIR).
  if (stat(MOUNT_POINT.c_str(), &sb) == 0 && S_ISDIR(sb.st_mode)) {
    ...
    // Use access() to check for read (R_OK), write (W_OK), and execute (X_OK) permissions.
    // For a FUSE mount point to be usable by the tests, these permissions are typically needed.
    ...
    return false; // Mount point exists but is not accessible with required permissions.
  }
  ...
  std::cerr << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
            ...
            << std::endl;
  return false; // Mount point is not ready (doesn't exist or isn't a directory).
}

// Verifies the integrity of data written by a specific thread within its designated block in the file.
// This function is a crucial part of the Random Write Test's verification phase. It checks if the
// content written by a single thread to its allocated region matches the expected content.
//
// Parameters:
//   payload: A string_view or const std::string& representing the portion of the file content
//            that contains the blocks written by all threads (typically, this excludes the global file header).
//   thread_id: The ID of the thread whose written block is to be verified. This determines
//              which specific segment of the payload and what expected content to check.
//   errors_found: A reference to a size_t counter that is incremented if any discrepancies
//                 are found. This allows the caller to accumulate a total error count.
//
// Logic:
//   1. Calculates the expected starting offset of the specified thread's block within the payload.
//   2. Iterates through each line that the thread was supposed to write.
//   3. For each line, it generates the expected content using `generate_line_content()`.
//   4. It then compares this expected content with the actual content extracted from the
//      `payload` at the calculated position for that line.
//   5. Logs detailed error messages if mismatches or out-of-bounds reads are detected.
//
// Returns:
//   true if the entire block for the specified thread_id is correct (all lines match expected content).
//   false if any mismatch is found or if there's an issue accessing the payload (e.g., out of bounds).
...
// Encapsulates the entire Append Test, verifying the correctness and atomicity of
// concurrent append operations to a single file.
//
// The test proceeds in several stages:
// 1. Setup: Ensures the mount point is ready and cleans up any previous test file.
//    Resets the barrier counter for appender threads.
// 2. Thread Launching: Creates and starts multiple `appender_thread_func` instances,
//    each of which will attempt to append lines to the shared file.
// 3. Execution: Waits for all appender threads to complete.
// 4. Verification: Reads the content of the append test file and performs checks:
//    a. Line Count Check: Verifies if the total number of lines in the file matches
...
//
// Returns:
//   true if all verification steps (correct line count and all lines present with
//   correct content) pass, indicating successful and atomic concurrent appends.
//   false otherwise.
bool run_append_test() {
  ...
  if (!check_mount_point_ready()) { // check_mount_point_ready already logs errors
    return false; // Cannot run test if mount point is not ready.
  }

  // Delete the append test file if it exists from a previous run to ensure a clean state.
  if (std::remove(FULL_APPEND_TEST_FILE_PATH.c_str()) == 0) {
    ...
    // If std::remove fails, check errno. ENOENT (No such file or directory) is acceptable,
    // as it means the file didn't exist, which is a clean state. Other errors might be problematic.
    if (errno != ENOENT) {
        std::cerr << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
                  << " TID: " << std::this_thread::get_id()
  ...
  // Reset the barrier counter (start_count) specifically for the appender threads.
  // This ensures the appender threads synchronize their start independently of the
  // random writer threads if they used the same barrier variables.
  {
    std::unique_lock<std::mutex> lk(start_mutex);
    start_count = 0;
  }
  ...
  // --- Thread Launching ---
  std::vector<std::thread> appender_threads; // Vector to hold appender thread objects.
  std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
            << " TID: " << std::this_thread::get_id()
            << "] AppendTest: Starting " << NUM_APPEND_THREADS << " appender threads, each writing "
            << NUM_LINES_PER_APPEND_THREAD << " lines." << std::endl;
  // Create and launch each appender thread. Each thread will execute appender_thread_func.
  ...
  // Line Count Check: Verify if the number of lines read matches the expected total.
  // This is a basic check for data completeness (no lost lines).
  ...
  for (int t = 0; t < NUM_APPEND_THREADS; ++t) {
    for (int l = 0; l < NUM_LINES_PER_APPEND_THREAD; ++l) {
      // Note: std::getline strips the newline, so expected lines are generated without it for comparison.
  ...
  // Why sorting? Concurrent appends (std::ios::app) ensure each write is atomic (goes to the end),
  // but the order in which lines from different threads appear in the file is non-deterministic.
  // Sorting allows us to verify that *all* expected data is present and correct, regardless of the
  // interleaved order resulting from concurrent execution.
  ...
  // Only compare content if line counts were potentially okay or for detailed error reporting.
  ...
  // 1. The line count check passed (`append_test_success` is still true from that).
  ...
  // If append_test_success was already false due to line count, that error message is primary.
...
// Orchestrates the execution of different test scenarios (Random Write, Append).
  ...
  // Calculate the total expected size of the file after all threads have written their content.
  // This includes the header and all lines from all threads.
  size_t expected_total_lines = NUM_THREADS * NUM_LINES_PER_THREAD;
  const off_t final_size =
      HEADER_LINE.size() +
      static_cast<off_t>(expected_total_lines) * (LINE_LENGTH + 1); // (LINE_LENGTH + 1 for newline)
  ...
  // 1. Simulates writing to pre-defined, fixed regions: The test aims to verify that threads
  //    can write to their specific byte offsets (`seekp()`) within a file without interference.
  //    Preallocating ensures these regions physically exist (or are at least accounted for by the FS)
  //    before threads attempt to write to them.
  // 2. Avoids file extension races: If the file were not preallocated, multiple threads
  //    writing with `seekp()` beyond the current end-of-file might trigger concurrent file extension
  //    operations. This could lead to race conditions, non-deterministic behavior, or performance
  //    anomalies that are not the primary focus of testing `seekp()` to *known* locations.
  // 3. Ensures distinct blocks: Preallocation helps solidify the concept of each thread "owning"
  ...
  // Calculate the expected size of the payload (all lines written by all threads).
  ...
  size_t bytes_read = static_cast<size_t>(infile.gcount()); // Get actual number of bytes read.
  ...
  if (bytes_read != remaining_bytes) {
    ...
    // If fewer bytes were read than expected, it's a strong indicator of data loss or truncation.
    // The block verification below might operate on incomplete data or fail.
    if (bytes_read < remaining_bytes) {
        // Safeguard: Resize file_payload to actual bytes read to prevent substr from
        // going out of bounds in verify_thread_block if the read was short.
  ...
  // This step checks if each thread wrote its data into its correct, designated region of the file.
  std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
            << " TID: " << std::this_thread::get_id()
            << "] Main: Starting block-level data verification." << std::endl;
  bool block_level_verification_failed = false; // Flag to track if any block is incorrect.
  size_t block_verification_errors = 0; // Counter for errors found by verify_thread_block.

  // Defensive check: If the payload length is unexpectedly short even if gcount initially matched,
  // it signals a critical issue. This should ideally not happen if infile.read and gcount are consistent.
  if (file_payload.length() < remaining_bytes && bytes_read == remaining_bytes) {
    ...
    block_level_verification_failed = true;
      ...
      // Iterate through each thread and verify its corresponding block in the payload.
      for (int t = 0; t < NUM_THREADS; ++t) {
        if (!verify_thread_block(file_payload, t, block_verification_errors)) {
          block_level_verification_failed = true;
          // Continue checking all blocks to gather comprehensive error information,
          // rather than stopping at the first block error.
        }
      }
  }
  ...
  // This step checks if all expected lines of content are present *somewhere* in the file payload
  // and correctly formatted, irrespective of their exact position (which block-level verification covers).
  // It helps distinguish data loss/corruption from data misplacement.
  std::vector<std::string> lines_read_from_file;
  std::stringstream payload_stream(file_payload); // Use a stringstream to parse lines from the payload.
  ...
  // Sort both the read lines and expected lines, then compare them element by element.
  // This verifies that all data is present and correct, even if it got shuffled (which
  // block-level verification would have caught as an error).
  ...
  if (!content_match_success) {
    std::cout << "[FUSE CONCURRENCY LOG " << getFuseTestTimestamp()
              << " TID: " << std::this_thread::get_id()
              << "] VERIFICATION FAILED: Data integrity issues detected (sorted content mismatch)."
              << std::endl;
  }
  // --- End of Sorted Content Verification ---

  // --- SHA-256 Hash Verification ---
  // This is the most stringent and primary verification for the random write test.
  // It constructs the expected full file content (header + all lines in their original, intended order)
  // and compares its SHA-256 hash with the hash of the actual file content read from disk.
  // A matching hash indicates that the file on disk is byte-for-byte identical to the
  // perfectly constructed expected file, implying all concurrent writes were successful,
  // complete, and non-interfering at their designated pre-allocated offsets.
  ...
  // All three conditions must be true for the test to be considered fully passed:
  // 1. `hashes_match`: Confirms overall file integrity at the byte level. If this is true,
  //    the other two should generally also be true. It's the most comprehensive check.
  // 2. `content_match_success`: Confirms all expected content lines are present and correct
  //    (after sorting), guarding against data loss or corruption.
  // 3. `!block_level_verification_failed`: Confirms each thread's data is in its designated block,
  //    guarding against data misplacement.
  // While `hashes_match` implies the others, having separate checks provides more granular
  // debugging information in case of failures. For example, if hashes don't match,
  // the block-level or sorted-content checks can help pinpoint if it's due to misplacement,
