...
public:
  ...
  void setRaftNode(RaftNode *node) {
    raftNode_ = node;
    if (raftNode_) {
      raftNode_->setApplyFunc(
          [this](const std::string &c) { this->applyRaftCommand(c); });
    }
  }
  ...
  /** Compute and return the Merkle root of the current namespace. */
  std::string computeNamespaceRoot();

  /** Apply a single Raft log command to update metadata state. */
  void applyRaftCommand(const std::string &cmd);
