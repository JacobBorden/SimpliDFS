  ...
  inline static Message Deserialize(const std::string &data) {
    ...
    auto get_token = [&](const std::string &field_name) {
      if (!std::getline(iss, token, '|')) {
        if (iss.eof() &&
            iss.str().back() == '|') { // if data ends with delimiter, it means
                                       // the last field is empty
          token.clear();               // Treat as empty
          return;
        }
        throw std::runtime_error("Deserialize error: Stream ended prematurely "
                                 "or missing delimiter. Expected " +
    ...
    auto get_last_token = [&](const std::string &field_name) {
      if (!std::getline(
              iss, token)) { // Read the rest of the stream for the last field
        if (iss.eof() &&
            iss.str().back() == '|') { // if data ends with delimiter, it means
                                       // the last field is empty
          token.clear();               // Treat as empty
        ...
        // If stream is just ended without a trailing delimiter, it could be an
        // error or last field is empty However, getline sets eofbit if it reads
        // up to EOF. If token is empty and eof is true, it's an empty last
        // field. If token is not empty, it's a valid last field. If token is
        // empty and eof is not true (e.g. badbit or failbit), then it's an
        // error. For robustness, if getline fails and token is empty, assume
        // it's an empty last field if eof is set.
        if (iss.eof() && token.empty() && iss.str().back() != '|') {
          // This case means stream ended, and there was no final delimiter, and
          // no content for the last field. This is ambiguous. Could be a
          // malformed message missing its last field or an empty last field
          // without a trailing delimiter. Given current logic, if it's the very
          // last field, an empty token is acceptable.
