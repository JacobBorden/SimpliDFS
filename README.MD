...
- Fault tolerance framework including file replication strategy, detection of node failures, and automatic re-replication.
...
- **Node Management**: Core logic implemented (registration/heartbeat complete). Further enhancements for dynamic node property updates could be considered.
  - **Fault Tolerance**: Replication and re-replication are implemented via the networking library. Full end-to-end testing with a live networking layer is pending.
...
- FUSE 3 (libfuse) installed with static libraries (`libfuse3-dev` or built from source).
- c-ares development headers (`libc-ares-dev` or `c-ares-devel`).
...
1. Clone the repository:
   ...
   cd SimpliDFS
   ```
2. Run the setup script to install dependencies (it ensures Protobuf
   3.21.12 is installed, building from source if another version is
   present):
   ...
   - On openSUSE systems:
     ...
     ```
3. If you built a previous revision, remove the old `build` directory and any
   generated protobuf headers to avoid mixing versions:
   ```sh
   rm -rf build include/grpc/proto
   ```
4. Verify your `protoc` version is exactly 3.21.12:
   ```sh
   protoc --version
   ```
...
## Documentation
Additional design documents can be found in the [docs](docs/) directory. The [docs/overview.md](docs/overview.md) file provides a high-level architecture summary, the [docs/gcp_ci_cd.md](docs/gcp_ci_cd.md) guide covers deployment on Google Cloud, and API reference pages can be generated with Doxygen:

```sh
doxygen docs/Doxyfile
```

The generated HTML documentation will be placed in `docs/doxygen/html`.
